	...
	return (
		<div className="recommended">
			<div className="recommended__box">
				<h3 className="recommended__title title-3">Возможно вас заинтересует:</h3>
				<div className="recommended__navigation">
					<button className="recommended__btn recommended__btn-prev" type="button">{btnPrev}</button>
					<button className="recommended__btn recommended__btn-next" type="button">{btnNext}</button>
				</div>
			</div>
			<Swp className="recommended-swiper">{products.map((item, i) => <Product key={i} data={item} className="recommended-slide swiper-slide" />)}</Swp>
		</div>
	)
}